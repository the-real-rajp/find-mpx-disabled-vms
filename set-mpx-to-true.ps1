...
foreach ($vm in $vms) {
    ...
    if (!(Get-VMHardware -VM $vm -ErrorAction SilentlyContinue | Where-Object {$_.DeviceInfo.Label -eq 'MPX'} | Select-Object -ExpandProperty Enabled)) {
        # Enable MPX on the VM
        Set-VMHardware -VM $vm -Version v11 -MPXEnabled $true
...
$MPXEnabledVMs | Select-Object Name, @{Name='MPXEnabled'; Expression={'True'}} | Export-Csv -Path $CSVFile -NoTypeInformation
